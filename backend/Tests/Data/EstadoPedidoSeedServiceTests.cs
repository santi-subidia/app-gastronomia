...
public class EstadoPedidoSeedServiceTests
{
    ...
    // ---- Scenario: Fresh DB — all 9 states inserted with explicit IDs ----

    [Fact]
    public async Task SeedAsync_EmptyDb_InsertsNineWithIds()
    {
        // Arrange
        ...
        Assert.Equal("Contingencia", estados.First(e => e.Id == 9).Nombre);
    }
    ...
    // ---- Scenario: All 9 correct — no changes ----
    ...
    // ---- Scenario: Idempotent — running twice, still 9 rows ----
    ...
    // ---- Scenario: Inserts Devuelto and Contingencia when only 1-7 exist ----
    ...
    public async Task SeedAsync_Error_DoesNotRethrow()
    {
        ...
        // Assert: still exactly 9 rows, no crash
        var estados = await context.EstadosPedidos.ToListAsync();
        Assert.Equal(9, estados.Count);
    }
}
